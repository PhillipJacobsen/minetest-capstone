...
-- little manual supply scorpion earth occur law brick manage shoe midnight citys
...
minetest.register_on_modchannel_message(function(channel_name, sender, message)
    ...
    if (i == 1) then
        ...
        if (y ~= name) then
            print("no handler for this channel name")
            return
        elseif (sender ~= "") then
            print("crypto channel message received from another player instead of server")
            ...
            if (message.cmd == "tx_confirm") then
                if (message.msg.confirmations and message.msg.txid) then
                    if (message.msg.confirmations > 0) then
                        print("tx has " .. message.msg.confirmations .. " confirmations: ", message.msg.txid)
                        minetest.debug("tx has " .. message.msg.confirmations .. " confirmations: " .. message.msg.txid)
                    else
                        print("tx has not received any confirmations")
            ...
            if (message.cmd == "tx_response") then
                ...
                if (message.succeeded) then
                    print("Waiting for confirmation of tx: ", response.data.accept[1])
                    minetest.debug("Waiting for confirmation of tx: " .. response.data.accept[1])

                    --request confirmation of transaction
                    minetest.after(10, function()
                        if (player_channel:is_writeable()) then
                            ...
                            print("Error! Mod channel not writeable")
                        end
                    end)
                    ...
                    --local type = table.errors[invalid].type
...
local function select_online_players()
    ...
    local formspec = size ..
        ...
        "table[0,0.6;6.0,4.5;player_list;" -- "table[0,0.6;2.8,4.5;player_list;"
    ...
    if selected_player ~= "" then
        formspec = formspec
            .. "button[6.35,1;1.5,0.25;select;Select]" -- <X>,<Y>;<W>,<H>;<name>;<label>
    end
