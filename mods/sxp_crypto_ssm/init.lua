...
minetest.register_on_modchannel_message(function(channel_name, sender, message)
    ...
    if (i == 1) then
        ...
        --minetest.debug(modname .. " channel name: " .. channel_name)
        --minetest.debug(modname .. " sender name: " .. sender)
        if (y ~= sender) then
            ...
            if (msg.cmd and msg.msg) then
                ...
                elseif (msg.cmd == "confirm") then
                    ...
                    --minetest.debug(modname .. " received txid confirm message ")
                    http_api.fetch({
                        method = "GET",
                        --url = "https://api.solar.org/api/wallets/" .. msg.msg,
...
minetest.register_globalstep(function(dtime)
    --local pj_channel = minetest.mod_channel_join("pjchannel")
    --local writeable = pj_channel:is_writeable()
    --print("server writable: ",writeable)
    -- pj_channel:send_all("server globalstep")
    -- pj_channel:leave()
    --pj_channel = nil
    --print("server mod channel message")
end)
